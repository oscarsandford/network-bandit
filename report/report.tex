...
Exp3 is a variant of SoftMax, using the idea of Boltzmann Exploration and probability matching \cite{mab_algos}. Each arm $a$ of $k$ arms has an associated probability of 
being pulled at time $t$, $p_a(t) = (1-\gamma) \frac{w_a(t)}{\sum_{j=1}^k w_j(t)} + \frac{\gamma}{k}$. The single hyperparameter $\gamma$ indicates the learning 
rate. The weights associated with each action $a$ at time $t$ are noted $w_a(t)$.
...
Our baseline peer setup for this assessment does not use Algorithm~\ref{alg:alg1}, it is simply a set of 5 single-state peers we call $baseline$: $PeerArm(2,1)$, $PeerArm(4,1)$, 
...
\section{Results}
Each algorithm completes 100 runs, with a fixed number of steps each. The number of steps relates to how long the peer will be receiving data from its peers. It will be worth 
to consider how each algorithm fares in the short and long-term. To this end, we vary the number of steps in our experiments in order to better visualize this disparity.
...
discuss preliminary assumptions regarding the results, and then present plots containing visually definitive evaluations of each algorithm in our test environment.
...
and we therefore expected UCB to be stronger than $\varepsilon$-greedy. SoftMax was expected to be eponymously "softer" than $\varepsilon$-greedy. Exp3 was expected to be 
similar to SoftMax due to it being derivative of the SoftMax probability drawing method.

% Details on experiments and test results.
\subsection{Experimental Results} 
Text here.
% Start with baseline 5 peers.
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{Figures/base5peers_1000step_5algos.png}
    \caption{Average reward over time for each algorithm over 1000 steps using $baseline$ peers.}
    \label{fig:base5peers_1000step_5algos}
\end{figure}
...
% Does softmax eventually overtake eps-greedy? Baseline 5 peers case vs. n=10 gen peers case.
